...
// than leaving the user on an indefinite spinner (mirrors the swap result).
const SLOW_SETTLEMENT_MS = 20_000

type EarnWithdrawResultProps = {
  ...
  if (status === 'in-progress') {
    return (
      <div className="mt-6 flex flex-col items-center justify-center py-10 text-center">
        ...
        <h1 className="mt-6 text-2xl font-semibold tracking-tight">Moving to Available…</h1>
        <p className="mt-2 text-sm text-muted-foreground">This can take a few minutes.</p>
        {slow && (
          <div className="mt-6 flex flex-col items-center gap-3 animate-fade-in">
            <p className="max-w-xs text-xs text-muted-foreground">
              This is taking longer than usual. It&apos;s safe to leave this page — your withdrawal will keep
  ...
  return (
    <div className="mt-6 flex flex-col animate-fade-in">
      ...
      <SurfaceCard className="mt-8 flex flex-col gap-0 p-0">
        <Row size="md" className="px-4 py-3 border-b border-border" label="From" value={protocol} />
        <Row
          size="md"
